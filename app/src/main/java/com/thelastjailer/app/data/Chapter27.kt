 ...
 * shape. The thread pulled from the Sundering Ground reacts at night, and the whole — now aware,
 * through its Duskmere foothold, of exactly what was taken and what it might be able to locate — sends
 * its most direct incursion yet to reclaim it. Twenty-seventh and toughest combat yet (The
 * Reclamation, 370 HP), and for the first time not a fragment, a straggler, or a haunted ward, but a
 * piece of the whole's own attacking body, deployed rather than merely persuading. Ends with a genuine
 * gain rather than another ambiguity: repelling the assault leaves a splinter of the whole itself in
 * Kaelen's hands, and the thread, having been fought over, finally shows a direction rather than just
 * a memory — the first real lead on the true, still-unnamed sixth gate.
 */
val chapter27Nodes: List<StoryNode> = listOf(
    StoryNode(
        ...
        narrativeText = """
            ...
            The prisoner is awake too, and has been for a while. "It's not the thread doing that,"
            it says. "Something's noticed we're holding it."
        """.trimIndent(),
        choices = listOf(
            Choice(
                label = "\"Noticed how? Say it plainly.\"",
                nextNodeId = "what_the_prisoner_warns",
    ...
    StoryNode(
        id = "what_the_prisoner_warns",
        chapterId = "chapter_27",
        title = "What the Prisoner Warns",
        illustrationId = "what_the_prisoner_warns",
        narrativeText = """
            "The foothold it took through Wren runs both directions, remember. It saw us pull
            that thread free the same way I saw the Sundering Ground through it. It knows what
            that thread might be able to point toward, and it knows we don't understand it yet.
            It's not going to let us catch up."
        """.trimIndent(),
        choices = listOf(
    ...
    StoryNode(
        ...
        narrativeText = """
            ...
            so much as take — every strike aimed less at him than at the pouch at his belt, at
            the thread inside it, at the one small thing in this fight it actually came for.
        """.trimIndent(),
        choices = emptyList(),
    ...
    StoryNode(
        ...
        narrativeText = """
            ...
            anything, too. Those two things happening on the same night feels like it should
            mean something."

            It does. The prisoner confirms it before Kaelen can ask: the thread's vision was real,
            the sixth gate exists, and for the first time since this began, they know roughly
            where to start looking instead of waiting for the next silence, the next yes, the
            next thing lost before they arrive.
